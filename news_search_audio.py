...
def convert_text_to_voice(speech_file_path, api_response):
    client = OpenAI()
...
def render_suggestions():
    ...
    suggestions = [
        ...
        "Top leisure news in Singapore",
        "Current financial news in the UK",
    ]
...
mic_recorder(key='my_recorder', 
                callback=callback,    
                start_prompt="Search for news, speak out your query by clicking this button",
                stop_prompt="End your query by clicking the stop button",
...
placeholders[0].info(f"{items}")

header.write("Search finished. Try something else!")

speech_file_path = 'audio_response.mp3'
convert_text_to_voice(speech_file_path, items)
